...
def mut_cross(MF_para_H, MCR_para_H, bounds, j, pop_size, obj_list_G, populations_G, P_i_int, Archive, target, dims, seed):
    ...
    Fi = -1.0
    while Fi <= 0.0:
        Fi = stats.cauchy.rvs(loc = MCR_para_H, scale = math.sqrt(0.1), size = 1, random_state = seed)
    ...
    trial = np.zeros(dims)
    CRi = stats.norm.rvs(loc = MCR_para_H, scale = math.sqrt(0.1), size = 1, random_state = seed)
